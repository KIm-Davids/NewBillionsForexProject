...
const FeatureSection = () => {
  useEffect(() => {
    if (typeof window !== 'undefined') {
      if (window.location.hash === '#feature-section') {
        ...
        {features.map((feature, index) => (
          <div key={index} className="w-full sm:1/2 lg:w-1/3">
            <div className="flex ml-5">
              <div
                className="flex mx-6 h-10 w-10 p-2 bg-neutral-900 text-blue-600 justify-center items-center rounded-full"
              >
                {feature.icon}
              </div>
              <div>
                <h5 className="mt-1 mb-6 text-xl">{feature.text}</h5>
                <p className="text-xl p-2 mb-20 text-neutral-500">{feature.description}</p>
              </div>
            </div>
